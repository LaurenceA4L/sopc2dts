...
"""Port of sopc2dts.lib.components.base.SICSpiMaster."""
...
class SICSpiMaster(BasicComponent):
    """
    SPI master — adds #address-cells, #size-cells and slave sub-nodes.
    Port of sopc2dts.lib.components.base.SICSpiMaster.
    """
    ...
    def to_dt_node(self, board_info: object, conn: Optional["Connection"]) -> object:
        ...
        bic = board_info.get_bic_for_chip(self.instance_name)
        if bic is not None and hasattr(bic, "slaves") and bic.slaves:
            node.add_property(DTProperty("#address-cells", 1))
            node.add_property(DTProperty("#size-cells", 0))
            for slave in bic.slaves:
