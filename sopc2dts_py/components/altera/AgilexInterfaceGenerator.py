...
"""
Port of the Agilex7 interface generator handler.

intel_agilex_interface_generator appears as a sub-component of intel_agilex_hps
in Platform Designer.  It carries the EMAC signal interfaces that need to be
moved to the HPS EMAC component instances before the system tree is generated.
Mirrors A10InterfaceGenerator but targets intel_agilex_hps.
"""
...
class AgilexInterfaceGenerator(BasicComponent):
    """
    Moves EMAC interfaces from the Agilex interface generator to their
    associated HPS EMAC component.
    Mirrors A10InterfaceGenerator for intel_agilex_hps.
    """
    ...
    def _move_emacs(self, sys: "AvalonSystem") -> None:
        hps_name = None
        for hps in sys.get_components_by_class("intel_agilex_hps"):
            if self.instance_name.startswith(hps.instance_name + "_"):
                hps_name = hps.instance_name
                break
        if hps_name:
            logger.debug("AgilexInterfaceGenerator: HPS name is %s", hps_name)
        for i in range(3):
            ...
            if emac is None:
                logger.debug("AgilexInterfaceGenerator: could not find %s", emac_name)
                continue
            for suffix in _EMAC_SIGNAL_SUFFIXES:
                ...
                if intf is not None:
                    logger.debug(
                        "AgilexInterfaceGenerator: moving %s to %s",
